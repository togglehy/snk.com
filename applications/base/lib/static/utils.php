...
class utils
{
    public static function match_network($nets, $ip, $first = false)
    {
        $return = false;
        if (!is_array($nets)) {
            $nets = array($nets);
    ...
    public static function parse_str_size($str)
    {
        if (preg_match('/^([0-9]+)\s*([gmk]|)$/i', $str, $match)) {
    ...
    public static function _apath(&$array, $path, &$ret)
    {
    ...
    public static function apath(&$array, $map)
    {
    ...
    public static function unapath(&$array, $col, $path, &$ret)
    {
    ...
    public static function array_path($array, $path)
    {
    ...
    public static function remove_p($sDir)
    {
                ...
                if ($sItem != '.' && $sItem != '..') {
                    if (is_dir($sDir.'/'.$sItem)) {
                        ...
                        if (!unlink($sDir.'/'.$sItem)) {
                            trigger_error('因权限原因 '.$sDir.'/'.$sItem.'无法删除', E_USER_NOTICE);
                        }
                    }
                }
            }
    ...
    public static function replace_p($path, $replace_map)
    {
        if (is_dir($path)) {
            $obj = dir($path);
            while (($file = $obj->read()) !== false) {
    ...
    public static function replace_in_file($file, $replace_map)
    {
    ...
    public static function mydate($f, $d = null)
    {
        global $_dateCache;
        if (!$d) {
            $d = time();
    ...
    public static function steprange($start, $end, $step)
    {
        if ($end - $start) {
            if ($step < 2) {
                $step = 2;
    ...
    public static function array_ksort_recursive($data, $sort_flags = SORT_STRING)
    {
        if (is_array($data)) {
    ...
    public static function array_md5($array, $sort_flags = SORT_STRING)
    {
    ...
    public static function _filter_input($data)
    {
        if (is_array($data)) {
    ...
    //过滤CRLF注入攻击
    public static function _filter_crlf($url)
    {
        $url = trim($url);
    ...
    public static function encrypt($data)
    {
        ...
        $encrypt = mcrypt_encrypt(MCRYPT_DES, VMCSHOP_TOKEN, $prep_code, MCRYPT_MODE_ECB);
    ...
    public static function decrypt($str)
    {
        ...
        $str = mcrypt_decrypt(MCRYPT_DES, VMCSHOP_TOKEN, $str, MCRYPT_MODE_ECB);
        $block = mcrypt_get_block_size('des', 'ecb');

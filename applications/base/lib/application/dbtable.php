...
class base_application_dbtable extends base_application_prototype_filepath{
    ...
    function real_table_name(){
        return vmc::database()->prefix.$this->target_app->app_id.'_'.$this->key();
    }
    ...
    function type_define($type){
        if(!self::$__type_define){
            if(defined('EXTENDS_DIR') && file_exists(EXTENDS_DIR.'/base/datatypes.php')){
                 require(EXTENDS_DIR.'/base/datatypes.php');
            }else{
    ...
    private function get_current_define($tbname){
        ...
        if($define){
            $rows = @vmc::database()->select('SHOW COLUMNS FROM '.$tbname);
            $columns = array();
            if($rows){
                foreach($rows as $c){
            ...
            $rows = @vmc::database()->select('SHOW INDEX FROM '.$tbname);
            $index = array();
            if($rows){
                foreach($rows as $row){
    ...
    function clear_by_app($app_id){
        $db = vmc::database();
        $rows = $db->select('show tables like '.$db->quote(vmc::database()->prefix.$app_id.'\_%'));
        foreach($rows as $row){
            $tables[] = current($row);
        }
        if($tables){

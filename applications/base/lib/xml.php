...
class base_xml
{
    public $_xsd_parser = null;
    public $_path = null;

    public function array2xml(&$value, $name, $xsd = null)
    {
    ...
    public function _array2xml_with_xsd(&$value, $name)
    {
        $body = '';
        $attrs = array();
        if (is_array($value)) {
    ...
    public function xml2array(&$xmldata, $xsd = null)
    {
        $this->init_xsd($xsd);
        ...
        foreach ($tags as $i => $tag) {
            if ($tag['type'] == 'complete' || $tag['type'] == 'open') {
                $elements[$i] = array('tag' => $tag['tag'],'element' => isset($tag['attributes']) ?
                    $tag['attributes'] : null, );
                if (isset($tag['value'])) {
                    if (isset($this->_schema['elements'][$tag['tag']]['_attrs'])) { //有属性
    ...
    public function init_xsd($xsdfiles)
    {

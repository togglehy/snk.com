...
class base_view_helper
{
    ...
    public function block_capture($params, $content, &$tpl)
    {
        if (null !== $content) {
    ...
    public function block_area($params, $content, &$tpl)
    {
        if (null !== $content) {
    ...
    public function block_t($params, $content, &$tpl)
    {
    ...
    public function _tpl_escape_chars($string)
    {
        if (!is_array($string)) {
    ...
    public function function_appconf($params, &$tpl)
    {
     ...
     *           http://smarty.php.net
     */
    public function function_counter($params, &$tpl)
    {
        static $count = array();
    ...
    public function function_html_options($params, &$tpl)
    {
        $name = null;
        $options = null;
        $selected = array();
    ...
    public function _tpl_function_html_options_optoutput(&$tpl, $key, $value, $selected)
    {
        if (!is_array($value)) {
    ...
    public function _function_html_options_optgroup(&$tpl, $key, $values, $selected)
    {
    ...
    public function function_html_table($params, &$template_object)
    {
        $table_attr = 'border="1"';
        $tr_attr = '';
        $td_attr = '';
    ...
    public function _function_html_table_cycle($name, $var)
    {
        static $names = array();
    ...
    public function function_json($params, &$smarty)
    {
        return json_encode($params['from']);
    }

    public function function_mailto($params, &$template_object)
    {
        extract($params);
    ...
    public function function_pagers($params, &$smarty)
    {
        ...
        $prev = $params['data']['current'] > 1 ?
            '<a href="'.str_replace($params['data']['token'], $params['data']['current'] - 1, $params['data']['link']).'" class="prev" onmouseover="this.className = \'onprev\'" onmouseout="this.className = \'prev\'" title="'.'上一页'.'">'.'上一页'.'</a>' :
            '<span class="unprev" title="'.'已经是第一页'.'">'.'已经是第一页'.'</span>';

        $next = $params['data']['current'] < $params['data']['total'] ?
            '<a href="'.str_replace($params['data']['token'], $params['data']['current'] + 1, $params['data']['link']).'" class="next" onmouseover="this.className = \'onnext\'" onmouseout="this.className = \'next\'" title="'.'下一页'.'">'.'下一页'.'</a>' :
...
EOF;
    ...
    public function _pager_link($from, $to, $l, $p, $c = null)
    {
        for ($i = $from;$i < $to + 1;$i++) {
            if ($c == $i) {
    ...
    public function function_toinput($params, &$smarty)
    {
        $html = null;
    ...
    public function _tpl_function_toinput($data, &$ret, $path = null)
    {
        foreach ($data as $k => $v) {
            $d = $path ? $path.'['.$k.']' : $k;
            if (is_array($v)) {
    ...
    public function modifier_amount($money, $currency = null, $basicFormat = false, $chgval = true)
    {
    ...
    public function modifier_storager($image_id, $size = '')
    {
        
        return base_storager::modifier($image_id, $size);
    }

    public function modifier_cdate($string, $type = 'FDATE_STIME')
    {
    ...
    public function modifier_date($string, $format = 'r', $default_date = null)
    {
        if ($string != '') {
    ...
    public function modifier_date_format($string, $format = '%b %e, %Y', $default_date = null)
    {
        if ($string != '') {
    ...
    public function modifier_escape($string, $esc_type = 'html', $char_set = 'ISO-8859-1')
    {
        switch ($esc_type) {
        case 'html':
    ...
    public function modifier_gender($result)
    {
        switch ($result) {
        case 'male':
            return '男';
    ...
    public function function_testtest($params, &$tpl)
    {
        return $params['content'];
    }//End Function

    public function modifier_t($content, $app_id, $arg1 = null)
    {
        return $content;
    }//End Function

    public function modifier_number($num, $type = 0)
    {
        switch ($type) {
        case 0:
            $number = intval($num);
    ...
    public function modifier_regex_replace($string, $search, $replace)
    {
    ...
    public function modifier_replace($string, $search, $replace)
    {
        return str_replace($search, $replace, $string);
    }

    public function modifier_strip($string, $replace = ' ')
    {
        return preg_replace('!\s+!', $replace, $string);
    }

    public function modifier_styleset($style)
    {
        switch ($style) {
        case 1:
    ...
    public function block_strip($params, $content, &$tpl)
    {
        if (null !== $content) {
    ...
    public function modifier_print_r($string)
    {
        echo '<pre>';
        print_r($string);
    }

    public function modifier_count($array)
    {
        return count($array);
    }
}

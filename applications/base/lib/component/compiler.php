...
class base_component_compiler
{
    ...
    public $enable_strip_whitespace = false;
    public $_vars = array();
    ...
    public $_block_stack = array();
    public $_head_stack = array();
    public $bundle_vars = array();
    public $compile_helper = array();
    public $view_helper = array();
    public function __construct(&$controller)
    {
        $this->controller = $controller;
        foreach (vmc::servicelist('view_compile_helper') as $helper) {
        ...
        foreach (vmc::servicelist('view_helper') as $helper_path => $helper) {
            foreach (get_class_methods($helper) as $method) {
                $this->set_view_helper($method, $helper_path);
            }
        }
    }
    public function set_compile_helper($method, &$helper)
    {
        $this->compile_helper[$method] = $helper;
    }
    public function set_view_helper($method, $helper_path)
    {
        $this->view_helper[$method] = $helper_path;
    }
    public function compile_file($file_path)
    {
    ...
    public function &compile($file_contents)
    {
        $this->_if_stack = '';
        $compiled_text = '';
        $this->_block_stack = array();
        foreach ((array) vmc::servicelist('view_compile_prefilter') as $object) {
    ...
    public function post_compile(&$compiled_text)
    {
        foreach ((array) vmc::servicelist('view_compile_postfilter') as $object) {
            $file_contents = $object->process($func, $file_contents);
        }
        ...
        if (count($matchs[1])) {
            ...
            foreach ($helpers as $helper) {
                $compiled_text = '<?php $this->__view_helper_model[\''.$helper.'\'] = vmc::singleton(\''.$helper.'\'); ?>'.$compiled_text;
            }
        ...
        $compiled_text = preg_replace(array(
            '/\<\?php\s*\?\>/',
            '/\?\>\s*\<\?php/',
        ), '', $compiled_text);
    }
    public function _parse_function($function, $arguments, $bundle_var_only = false)
    {
        switch ($function) {
            case 'ldelim':
...
?>';
    ...
    public function tpl_compile_include($arguments, &$object)
    {
        ...
        if ('true' == $is_theme) {
            $include_file = "'".app::get('site')->getConf('current_theme').'/'.str_replace(array(
                '"',
                '\'',
            ), '', $include_file)."'";
        }
    ...
    public function _arguments_if($arguments, $bundle_var_only)
    {
        if (!$this->_if_replace) {
            $to_replace = array(
                'is\s+not\s+odd' => '%2==0',
    ...
    public function _parse_arguments($arguments, $bundle_var_only = false)
    {
    ...
    public function _prepare_fix_quote($match)
    {
        $this->_fix_quotes[$this->_fix_quotes_seq] = $match[0];

        return '_!ok'.($this->_fix_quotes_seq++).'!_';
    }
    public function _restone_fix_quote($match)
    {
        return $this->_fix_quotes[$match[1]];
    }
    public function _begin_fix_quote(&$variable)
    {
        $this->_fix_quotes_seq = 0;
        $this->_fix_quotes = array();
        $variable = preg_replace_callback('/([\'"]).*?(?:[^\\\\]|)\1/u', array(&$this,
            '_prepare_fix_quote',
        ), $variable);
    }
    public function _end_fix_quote(&$variable)
    {
        if ($this->_fix_quotes) {
            $variable = preg_replace_callback('/_!ok([0-9]+)!_/u', array(&$this,
                '_restone_fix_quote',
            ), $variable);
        }
    }
    public function getRuntimeFunc($function, $real = true)
    {
        if (isset($this->view_helper[$function])) {
            if ($real === true) {
                return 'vmc::singleton(\''.$this->view_helper[$function].'\')->'.$function;
            } else {
    ...
    public function _fix_modifier($variable, $fix_quote = true, &$bundle_var_only)
    {
        if (strpos($variable, '|')) {
            if ($fix_quote) {
                $this->_begin_fix_quote($variable);
            }
            ...
            if ($fix_quote) {
                $this->_end_fix_quote($variable);
            }
        }

        return $variable;
    }
    public function _parse($cmd_line, &$bundle_var_only)
    {
        $this->i = 0;
        $this->lib = array();
        ...
        $res = preg_replace_callback('!(\$[a-z0-9\_\.\$\[\]\"\\\']+)!isu', array(&$this,
            '_in_str',
        ), $cmd_line);
        foreach ($this->lib as $i => $var) {
            ...
            if ($p = strpos($var, '.')) {
                ...
                if ($first == '$env') {
                    ...
                    switch ($second) {
                        ...
                        case 'BASE_FULL_URL':
                            $var = '';
                            $var_ns = 'vmc::base_url(1)';
                        break;
                        case 'STATICS_HOST_URL':
                            $var = '';
                            $var_ns = 'vmc::get_app_statics_host_url()';
                        break;
                        case 'NOW':
                            $var = '';
                            $var_ns = 'time()';
            ...
            $var = preg_replace_callback('!"_s([0-9]+)s_"!', array(&$this,
                '_rest',
            ), $var);
            $this->lib[$i] = $var;
        }
        $res = preg_replace_callback('!"_s([0-9]+)s_"!', array(&$this,
            '_rest_root',
        ), $res);
        $this->lib = array();
    ...
    public function _rest_root($match)
    {
        return $this->lib[$match[1]];
    }
    public function _rest($match)
    {
        return $this->lib[$match[1]];
    }
    public function _in_str($varstr)
    {
        $varstr = $varstr[1];
        $varstr = preg_replace_callback('/\[(\$[a-z0-9\_\.]+)\]/iu', array(&$this,
            '_in_sub_str',
        ), $varstr);
    ...
    public function _in_sub_str($varstr)
    {
        $varstr = $varstr[1];
        $varstr = preg_replace_callback('/\[(\$[a-z0-9\_\.]+)\]/iu', array(&$this,
            __METHOD__,
    ...
    public function strip_whitespace($tpl_source)
    {
    ...
    public function _compile_ui_function($function, $arguments, &$_result)
    {
    ...
    public function _compile_compiler_function($function, $arguments, $bundle_var_only, &$_result)
    {
    ...
    public function _compile_custom_function($function, $arguments, &$_result)
    {
    ...
    public function _compile_custom_block($function, $arguments, &$_result)
    {
        if ($function{0} == '/') {
            $start_tag = false;

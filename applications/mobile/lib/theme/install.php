...
class mobile_theme_install
{
    ...
    {
        if (!is_dir(THEME_M_DIR)) {
            utils::mkdir_p(THEME_M_DIR);
        }
    } //End Function

    public function monitor_change($theme){
        $config_file_path = THEME_M_DIR.'/'.$theme.'/config.xml';
    ...
    public function remove_theme($THEME_M_DIR)
    {
        $dir = THEME_M_DIR.'/'.$THEME_M_DIR;
    ...
    public function install($file, &$msg)
    {
        ...
        if (is_dir(THEME_M_DIR.'/'.$filename)) {
            $msg = '已存在模板包：'.$filename;
            return false;
        }
        $new_THEME_M_DIR = $this->__build_dir(str_replace('\\', '/', THEME_M_DIR.'/'.$filename));
        if (!$tar->openTAR($file['tmp_name'], $new_THEME_M_DIR) ||!$tar->containsFile('config.xml') ) {
            $msg = '错误的模板包';
            return false;
        }

        foreach ($tar->files as $id => $file) {
            $fpath = $new_THEME_M_DIR.'/'.$file['name'];
        ...
        if (!$config = $this->init_theme($filename)) {
            $this->__remove_dir($new_THEME_M_DIR);
            $msg = '模板包创建失败';
            return false;
        }
    ...
    private function __remove_db_theme($theme)
    {
        ...
        return app::get('mobile')->model('themes_file')->delete($filter);
    }
    private function __remove_dir($sDir)
    {
        if ($rHandle = opendir($sDir)) {
            while (false !== ($mobilem = readdir($rHandle))) {
                if ($mobilem != '.' && $mobilem != '..') {
                    if (is_dir($sDir.'/'.$mobilem)) {
                        $this->__remove_dir($sDir.'/'.$mobilem);
                    } else {
                        if (!unlink($sDir.'/'.$mobilem)) {
                            trigger_error('因权限原因，模板文件'.$sDir.'/'.$mobilem.'无法删除', E_USER_NOTICE);
                        }
                    }
                }
            }
    ...
    {
        if ($dh = opendir(THEME_M_DIR)) {
            while (($file = readdir($dh)) !== false) {
                if (substr($file, -4, 4) == '.tgz') {
                    //$filename_arr[] = $file;
                    $theme_file['tmp_name'] = THEME_M_DIR.'/'.$file;
                    $theme_file['name'] = $file;
                    $theme_file['type'] = 'application/octet-stream';
                    $theme_file['error'] = '0';
                    $theme_file['size'] = filesize(THEME_M_DIR.'/'.$file);

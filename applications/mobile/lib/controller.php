...
class mobile_controller extends base_controller
{
    ...
    */
    final public function gen_url($params = array())
    {
        ...
        return app::get('mobile')->router()->gen_url($params);
    } //End Function
    /*
     * 设置主题模版类型
     * @var string $tmpl
    ...
    */
    final public function display_tmpl($tmpl, $fetch = false, $is_preview = false)
    {
        ...
        $this->_vars['mobile_theme_url'] = $this->_vars['base_url'].'/'.$this->get_theme();
        //title description
        $title = $this->title ?
        $this->title :app::get('mobile')->getConf('page_default_title');
        $keywords = $this->keywords ?
        $this->keywords :
        app::get('mobile')->getConf('page_default_keywords', $title);
        $description = $this->description ?
        $this->description :
        app::get('mobile')->getConf('page_default_description', $title);
        ...
        $tmpl_file = realpath(THEME_M_DIR.'/'.$this->get_theme().'/'.$tmpl);

        if (!$tmpl_file) {
            $tmpl = 'default.html';
        }
        $tmpl_content = vmc::singleton('mobile_theme_file')->get_tmpl_content($this->get_theme(), $tmpl);
    ...
    */
    final public function page($view, $no_theme = false, $app_id = null)
    {
        ...
        if ($no_theme == false && $current_theme) {
            ...
            $this->pagedata['_MAIN_M_'] = $view; //强制替换
            $this->pagedata['_THEME_M_'] = vmc::get_themes_m_host_url().'/'.$this->get_theme(); //模版地址
    ...
    */
     ...
     * @return void
    */
    public function end($result = true, $message = null, $url = null, $show_notice = false, $ajax = false)
    {
        if (!$this->transaction_start) {

...
class mobile_explorer_dbsave
{
    function __construct(){
        $this->fileObj = app::get('mobile')->model('themes_file');
        $this->ftype = array(
                    'html' =>'模板文件',
                    'gif'=>'图片文件',
                    'jpg'=>'图片文件',
    ...
    public function file_list($filter){
        ...
        $theme_dir = THEME_M_DIR.DIRECTORY_SEPARATOR.$this->theme;
        $cur_dir = $filter['dir'];
    ...
    public function backup_file($file) 
    {
        ...
        $baklist = $this->get_file_baklist(array('dir'=>dirname(THEME_M_DIR.DIRECTORY_SEPARATOR.$this->theme.'/'.$file), 'show_bak'=>true, 'type'=>'all'), $file);
        ...
        if($fileInfo['extension']=='css'||$fileInfo['extension']=='jpg'||$fileInfo['extension']=='jpeg'||$fileInfo['extension']=='gif'||$fileInfo['extension']=='png'||$fileInfo['extension']=='bmp'){
            ...
            $storager = vmc::singleton('base_storager');

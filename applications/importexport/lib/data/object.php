...
class importexport_data_object
{
    ...
    public function get_content($filter, $offset, $limit)
    {
        ...
        $title = $this->title = $this->get_title();//需要导出字段
        //logger::info(implode(',', array_keys($title)));
        if ($this->group_col) {
    ...
    private function _pre_content($row)
    {
        ...
        foreach ((array) $row as $col => $val) {
            ...
            if (strpos((string) $cols[$col]['type'], 'table:') === 0 && $col != $this->group_col) {
                ...
                $textColumn = $hasModel->getRow($hasModel->schema['textColumn'], array($hasModel->schema['idColumn'] => $val));
                $val = $textColumn[$hasModel->schema['textColumn']] ? $textColumn[$hasModel->schema['textColumn']] : $val;
            }

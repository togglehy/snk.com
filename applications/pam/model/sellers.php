...
* account model类
*/
class pam_mdl_sellers extends dbeav_model{
	/**
	* 关联MODEL
	* @var array
	*/
	var $has_many = array(
        'account'=>'auth:append:seller_id^account_id',
    );
	/**
	* dump 等操作的相关联表
	* @var array
	*/
	 ...
	 */
	public function get_operactor_name($seller_id='')
	{
		if ($seller_id == '')
			return '未知或无操作员';

    	$data = $this->getList('login_type,login_account',array('seller_id'=>$seller_id));
		if (!$data)
		{
			return '未知或无操作员';
		}

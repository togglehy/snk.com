...
class b2c_user_vcode {
    ...
    public function send_sms($type ='signup', $mobile, $tmpl_data) {
        ...
        return vmc::singleton('b2c_messenger_stage')->action($tmpl_name, $tmpl_data, array('mobile'=>$mobile));
    }
    public function send_email($type = 'activation', $email, $tmpl_data) {
        if (!$action_name = $this->get_action_name($type)) return false;
        $sender_class = 'b2c_messenger_email';
        $tmpl_name = 'messenger:'.$sender_class.'/' . $action_name;

        return vmc::singleton('b2c_messenger_stage')->action($tmpl_name, $tmpl_data, array('email'=>$email));
    }
    ...
    public function mobile_login_verify($vcode, $mobile, $type) {
        if ('mobile'!=vmc::singleton('b2c_user_passport')->get_login_account_type($mobile)) {
            return true;
        }
        if ($this->verify($vcode, $mobile, $type)) {
            app::get('pam')->model('members')->update(array(
                'disabled' => 'false'
            ) , array(

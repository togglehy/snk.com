...
abstract class toauth_abstract
{
    ...
    protected function dologin($member_sdf,&$msg)
    {
        ...
        if(empty($openid) || trim($openid) == '')return false;
        ...
        if($pam_member && !empty($pam_member['member_id'])){
            if(!$this->bind_member($pam_member['member_id'],$msg)){
                return false;
            }
            return $pam_member['member_id'];
        }else{
            if(!$member_id = $this->create_member($member_sdf,$msg)){
                return false;
            }

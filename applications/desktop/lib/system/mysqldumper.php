...
class desktop_system_mysqldumper {
    ...
    public function multi_dump_sdf( $app,$bakdir ) {
        ...
        $sql = 'SELECT app_id FROM vmc_base_apps WHERE status=\'active\'';
        ...
        foreach($tables as $key => $tbl) {
            $app = $tbl['app_id'];
            $service = false;
            foreach ( vmc::servicelist('desktop_backup.'. $app) as $object ) {
                $service = true;

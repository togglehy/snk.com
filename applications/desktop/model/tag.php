            ...
            if($joinTable && $obj_id){
                if(!$where){
                    $sql = "select t.tag_id,t.tag_name,t.tag_type,count(o.{$obj_id}) as rel_count,$obj_id as ss,t.is_system
                     FROM vmc_base_tag t
                     LEFT JOIN vmc_base_tag_rel r ON r.tag_id=t.tag_id
                    ...
                    $sql = "select $obj_id as trel_id
                     FROM vmc_base_tag_rel r
                     LEFT JOIN {$joinTable} o ON r.rel_id=o.{$obj_id} and o.disabled!='true'
                     where r.tag_id = {$where}";
                }
            }else{
                $sql = "select t.tag_id,t.tag_name,t.tag_type,count(r.rel_id) as rel_count,t.is_system FROM vmc_base_tag t LEFT JOIN vmc_base_tag_rel r ON r.tag_id=t.tag_id where tag_type='$type' group by t.tag_id";
            }
        }else{
            $sql = "select * FROM vmc_base_tag where tag_type='$type'";
        }

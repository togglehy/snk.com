...
class site_router implements base_interface_router
{
    ...
    */
    public function get_separator()
    {
        if (!isset($this->__separator)) {
            $this->__separator = trim(app::get('site')->getConf('base_site_params_separator'));
        }
    ...
    */
    public function get_uri_expended_name($part = null)
    {
        if (!isset($this->__uri_expended_name)) {
            if (app::get('site')->getConf('base_enable_site_uri_expanded') == 'true') {
                $this->__uri_expended_name = '.'.app::get('site')->getConf('base_site_uri_expanded_name');
            } else {
    ...
    */
    public function http_status($code)
    {
        ...
        vmc::singleton('site_ctl_errorpage')->index($res);
        exit;
    ...
    */
    private function check_https()
    {
        ...
        if ($this->get_current_sitemap('use_ssl') === true && $_SERVER['SERVER_PORT'] == '80') {
            ...
            exit();
        } elseif ($this->get_current_sitemap('use_ssl') === false && $_SERVER['SERVER_PORT'] == '443') {

...
class site_theme_file_fsfile
{
    ...
    function bak_save($theme,$data){
        if(file_put_contents(THEME_DIR . '/' . $theme . '/config_bak.xml', $data)) {
            $flag = true;
        } else {
            $flag = false;
        }
    ...
    function get_full_file_url($theme, $file_content, $open_path, $file_name){
        return vmc::base_url(1) . rtrim(str_replace('//', '/', '/themes/' . $theme . '/' . str_replace(array('-','.'), array('/','/'), $open_path) . '/' . $file_name));
    }

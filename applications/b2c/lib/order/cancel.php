...
class b2c_order_cancel
{
    ...
    public function generate($sdf, &$msg = '')
    {
        ...
        //订单作废前验证
        foreach (vmc::servicelist('b2c.order.cancel.finish') as $service) {
            if (!$service->exec($order_sdf, $msg)) {
                return false;
            }
        }

        $order_sdf['status'] = 'dead';
        // 更新退款日志结果
        if ($mdl_order->save($order_sdf)) {
            ...
            ))->set_order_id($order_sdf['order_id'])->success('cancel', '订单已作废!', $order_sdf);
        } else {
            $msg = '作废失败!';
        ...
        foreach (vmc::servicelist('b2c.order.cancel.finish') as $service) {
            if (!$service->exec($order_sdf, $msg)) {
                //记录日志，不中断
                logger::error($sdf['order_id'].'作废时出错！'.$msg);
            }
        }

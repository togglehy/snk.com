...
class b2c_sales_order_aggregator_subselect extends b2c_sales_order_aggregator {
    ...
    public function getDefaultStandardData() {
        $aTemp = $this->getItem();
        $aStandard['attribute']['options'] = $this->getAttributes(array(
            'subgoods'
        ));
        $aStandard['operator']['options'] = $this->getOperators(array(
            'equal',
            'equal1'
        ));
        return array(
    ...
    protected function _view($aStandard, $html, $vpath, $level, $position, $is_auto) {
        ...
        $sAggregator.= '<span class="btn btn-lnk" onclick="showConditions(this)">[添加一个条件]</span>';
        if ($is_auto) {
    ...
    public function validate($cart_result, $aCondition) {
        ...
        $oCond = vmc::singleton($this->default);
        foreach ($cart_result['objects']['goods'] as $object) {
        ...
        return vmc::singleton($aOperator['object'])->validate($aCondition['operator'], $aCondition['value'], $total);
    }
}
?>

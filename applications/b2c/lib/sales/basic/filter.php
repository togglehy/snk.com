...
class b2c_sales_basic_filter
{
    ...
    protected function _init_operator() {
        if(is_null($this->_aOperator)) {
            $aResult = vmc::singleton('b2c_sales_basic_operator')->get_all();
    ...
    protected function _standard_view($aStandard,$vpath,$level,$position, $table_info=array()){
        ...
        return vmc::singleton('b2c_sales_basic_input_'.$aStandard['input'])->create($aStandard, $table_info);
    }
    ...
    public function _makeOptions($aData) {
        ...
        if(is_string($aData)) {
            ...
            switch($aData[0]) {
                case 'table':
                    $aResult = vmc::database()->select($aData[1]);
    ...
    // 预过滤的处理(prefilter)
    public function filter($aConditions){return false;}
    // 订单促销规则的处理(postfilter)
    public function validate($cart_objects, $condition){return false;}
    // postfilter
    public function valiate(){return false;}
    // 模板输出 虚函数
    public function view($aTemplate,$aData,$vpath,$level,$position,$is_auto) {return false;}
...
// 以下是模板要使用到的包含标签
     ...
     * @return string // html
     */
    function wrap_li($html,$level = 0,$position = -1){
        if($position<0){
            $root_class = ' root hide';
            $close_btn = "<span style='position:absolute;right:0;bottom:0;' class='btn btn-xs red' onclick=$(this).closest('.list-group-item').addClass('hide');><i class='fa fa-times'></i></span>";
        }
        return '<li class="list-group-item'.$root_class.'"  data-position="'.$position.'" >'.$html.$close_btn.'</li>';
    }
    ...
    function wrap_div($html,$level = 0,$position = null){
        return '<div class="c-pane">'.$html.'</div>';
    }
...
// 以下是自动配置模板要使用到的方法
    ...
    function create_remove(){
        return "<span style='position:absolute;right:0;bottom:0;' class='btn btn-xs red' onclick=$(this).closest('.list-group-item').remove();><i class='fa fa-trash-o'></i></span>";
    }
...
var makeConditions = function(el){
       ...
       new_item.load('index.php?app=b2c&ctl={$ctl}&act={$act}',{
           'condition':$(el).val(),

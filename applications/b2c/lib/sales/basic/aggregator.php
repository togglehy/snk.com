...
class b2c_sales_basic_aggregator extends b2c_sales_basic_filter {
    ...
    public function view($aTemplate, $aData, $vpath = 'conditions', $level = 0, $position = null, $is_auto = false) {
        ...
        foreach ($aConditions as $key => $row) {
            ...
            $sTemp = vmc::singleton($row['type'])->view($tTemplate, $tData, $spath, $level, $i, $is_auto);
            if ($sTemp) {
                $html.= $this->wrap_li($sTemp . $remove, $level, $i);
    ...
    protected function _view($aStandard, $html, $vpath, $level, $position, $is_auto) {
        ...
        if ($this->create_to_order) $sAggregator.= '<span class="btn btn-link" onclick="showConditions(this)"><i class="fa fa-plus"></i>新条件</span>';
    ...
    public function makeConditionOptions($type, $vpath, $level) {
        ...
        $html = <<<EOF
                 <select class="input-sm" onchange="makeConditions(this)" data-path="{$vpath}" data-level="{$level}">

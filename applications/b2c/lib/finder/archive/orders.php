...
class b2c_finder_archive_orders{
    ...
    //    $_is_all_return_ship = 1;
        ...
        // 判断是否使用了推广服务
        $is_bklinks = 'false';
        $obj_input_helpers = vmc::servicelist("html_input");
        ...
        $obj_action_button = vmc::servicelist('b2c_order.b2c_finder_orders');
        ...
        // 添加 html 埋点
        foreach( vmc::servicelist('b2c.order_add_html') as $services ) {
        ...
        $logisticst_service = vmc::service('b2c_change_orderloglist');
    ...
    private function get_goods_detail(&$aItems, &$order_items, &$gift_items, &$extend_items, $tml='admin_order_detail')
    {
        ...
        if ($aItems['order_objects'])
        {
            ...
            $arr_service_goods_type = vmc::servicelist('order_goodstype_operation');
                            ...
                            if ($item['item_type'] == 'product')
                            {
                                ...
                                if ($item['addon'])
                                {
                                    ...
                                    if ($item['addon']['product_attr'])
                                    {
                                        ...
                                        {
                                            $order_items[$k]['product']['name'] .= $arr_special_info['label'] . ('：') . $arr_special_info['value'] . ('、');
                                        }
                                        $order_items[$k]['product']['name'] = substr($order_items[$k]['product']['name'], 0, strrpos($order_items[$k]['product']['name'], ('、')));
                                        $order_items[$k]['product']['name'] .= ')';
                                    }
                                }
                            }
                            else
                            {
                                ...
                                if ($item['addon'])
                                {
                                    ...
                                    if ($item['addon']['product_attr'])
                                    {
                                        ...
                                        foreach ($item['addon']['product_attr'] as $arr_special_info)
                                        {
                                            $order_items[$k]['adjunct'][$index_adj]['name'] .= $arr_special_info['label'] . ('：') . $arr_special_info['value'] . ('、');
                                        }
                                        $order_items[$k]['adjunct'][$index_adj]['name'] = substr($order_items[$k]['adjunct'][$index_adj]['name'], 0, strrpos($order_items[$k]['adjunct'][$index_adj]['name'], ('、')));
                                        $order_items[$k]['adjunct'][$index_adj]['name'] .= ')';
                                    }
                                }
                            ...
                            if ($arr_service_goods_type_obj['gift'])
                            {
                                ...
                                if ($item['addon'])
                                {
                                    ...
                                    if ($item['addon']['product_attr'])
                                    {
                                        ...
                                        foreach ($item['addon']['product_attr'] as $arr_special_info)
                                        {
                                            $order_items[$k]['gifts'][$index_gift]['name'] .= $arr_special_info['label'] . ('：') . $arr_special_info['value'] . ('、');
                                        }
                                        $order_items[$k]['gifts'][$index_gift]['name'] = substr($order_items[$k]['gifts'][$index_gift]['name'], 0, strrpos($order_items[$k]['gifts'][$index_gift]['name'], ('、')));
                                        $order_items[$k]['gifts'][$index_gift]['name'] .= ')';
                                    }
                                }
    ...
    public function detail_logs($order_id){
        ...
        $render->pagedata['result'] = array('SUCCESS'=>('成功'),'FAILURE'=>('失败'));
        $pager = array(
            'current'=> $page,

...
class b2c_desktop_widgets_workcount implements desktop_interface_widget {
    ...
    function get_html($from,$to) {
        ...
        $member_comments = vmc::singleton('b2c_message_order');
        $b2c_message_disask = vmc::singleton('b2c_message_disask');
        $filter = array(
            'pay_status' => array(
                '0',
                '3'
        ...
        $alert_num_count = $mdl_products->db->select("select count(DISTINCT goods_id) as g_count from vmc_b2c_products where goods_type='normal' and store<='" . $alert_num . "'");
        ...
        $goods_notice = $member_goods->count(array(
            'type' => 'sto',
            'status' => 'ready'
        ));
        ...
        $today_filter = array(
            ...
            '_DTIME_' => array(
                'H' => array(
                    'createtime_from' => '00',
                    'createtime_to' => date('H')
                ) ,
                'M' => array(
                    'createtime_from' => '00',
                    'createtime_to' => date('i')
                )
            )
        );
        ...
        $yesterday_filter = array(
            ...
            '_DTIME_' => array(
                'H' => array(
                    'createtime_from' => '00',
                    'createtime_to' => date('H', $date)
                ) ,
                'M' => array(
                    'createtime_from' => '00',
                    'createtime_to' => date('i', $date)
                )
            )
        );
        ...
        //今日已付款订单
        $mdl_orders = $this->app->model('orders');
        $today_filter = array_merge($today_filter, array(
            'pay_status' => '1'
        ));
        ...
        //昨日已付款订单
        $date = strtotime('yesterday');
        $yesterday_filter = array_merge($yesterday_filter, array(
            'pay_status' => '1'
        ));
        ...
        $yesterday_filter = array(
            ...
            '_DTIME_' => array(
                'H' => array(
                    'regtime_from' => '00',
                    'regtime_to' => date('H', $date)
                ) ,
                'M' => array(
                    'regtime_from' => '00',
                    'regtime_to' => date('i', $date)
                )
            )
        );

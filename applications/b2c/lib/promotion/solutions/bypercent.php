...
class b2c_promotion_solutions_bypercent implements b2c_interface_promotion_solution {
    ...
    public function config($aData = array()) {
        if($this->type == 'goods'){
            return <<<EOF
    满<input class="input-sm input-xsmall" name="action_solution[b2c_promotion_solutions_bypercent][quantity]" required=true value="{$aData['quantity']}" />件，单件{$this->desc_pre}<input class="input-sm input-xsmall" name="action_solution[b2c_promotion_solutions_bypercent][percent]" required=true value="{$aData['percent']}" />{$this->desc_post}
EOF;

        }
        return <<<EOF
{$this->desc_pre}<input class="input-sm" name="action_solution[b2c_promotion_solutions_bypercent][percent]" required=true value="{$aData['percent']}" />{$this->desc_post}
EOF;
    ...
    public function setString($solution) {
        $this->description = $this->desc_pre . $solution['percent'] . $this->desc_post;
    }

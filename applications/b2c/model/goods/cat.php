...
class b2c_mdl_goods_cat extends dbeav_model {
    ...
    public function getTree() {
        return $this->db->select('SELECT o.cat_name AS text,o.cat_id AS id,o.parent_id AS pid,o.p_order,o.cat_path,
                    is_leaf as type,o.child_count FROM vmc_b2c_goods_cat o
                    ORDER BY o.cat_id');
    }
    /**
     * 注册商品分类的meta
    ...
    public function getCatParentById($id, $view = 'index') {
        ...
        if (is_array($id)) {
            ...
            $result = $this->getList('cat_id,cat_name', array(
                'parent_id|in' => $id
            ) , 0, -1, 'p_order,cat_id ');
        } else {
            $result = $this->getList('cat_id,cat_name', array(
        ...
        foreach ($result as $cat_key => $cat_value) {
            $result[$cat_key]['link'] = app::get('site')->router()->gen_url(array(
                'app' => 'b2c',
                'ctl' => 'site_gallery',
                'args' => array(
                    $cat_value['cat_id'],
    ...
    public function getMap($depth = - 1, $cat_id = 0) {
        ...
        if ($cat_id > 0) {
            $row = $this->getList('cat_path', array(
                'cat_id' => intval($cat_id)
            ));
            if ($depth > 0) {
                $depth+= substr_count($row['cat_path'], ',');
            }
            $rows = $this->getList('cat_name,cat_id,parent_id,is_leaf,cat_path,type_id', array(
                'cat_path|head' => $row['cat_path'] . $cat_id
            ) , 0, -1, 'cat_path,p_order ASC');
        } else {
    ...
    public function getPath($cat_id) {
        ...
        if ($row['cat_path'] && $row['cat_path'] != ',') {
            $rows = $this->getList('cat_name,cat_id', array(
                'cat_id|in' => explode(',', substr(substr($row['cat_path'], 0, -1) , 1))
            ) , 0, -1, 'cat_path DESC');
            foreach ($rows as $row) {
    ...
    function _map(&$cat_cuttent, $data, $key) {
        if (is_array($data[$key])) {
            foreach ($data[$key] as $k => $v) {
                $cat_cuttent[] = $v;
                if ($data[$v['cat_id']]) $this->_map($cat_cuttent, $data, $v['cat_id']);
            }
        }
    }
    ...
    function getTypeList() {
        ...
        return $obj_goods_type->getList('type_id,name', array(
            'disabled' => 'false'
        ));
    }
    ...
    function get_cat_list($show_stable = false) {
        if (base_kvstore::instance('b2c_goods')->fetch('goods_cat.data', $contents) !== false) {
            if (is_array($contents)) $result = $contents;
            else $result = json_decode($contents, true);
            if ($result) {
                if ($show_stable) {
                ...
                foreach ($result as $k => $val) {
                    $result[$k]['url'] = app::get('site')->router()->gen_url(array(
                        'app' => 'b2c',
                        'ctl' => 'site_gallery',
                        'args' => array(
                            $val['cat_id']
    ...
    function toRemove($catid, &$msg = '') {
        $aCats = $this->getList('*', array(
            'parent_id' => intval($catid)
        ));
        if ($aCats && count($aCats) > 0) {
            //trigger_error(('删除失败：本分类下面还有子分类'), E_USER_ERROR);
            $msg = '删除失败：本分类下面还有子分类';
            return false;
        }
        $obj_goods = $this->app->model('goods');
        $aGoods = $obj_goods->getList('goods_id', array(
            'cat_id' => intval($catid) ,
            'disabled' => 'false'
        ));
        if ($aGoods && count($aGoods) > 0) {
            //trigger_error(('删除失败：本分类下面还有商品'), E_USER_ERROR);
            $msg = '删除失败：本分类下面还有商品';
            return false;
        }
        //$row = $this->db->selectrow('SELECT parent_id FROM vmc_b2c_goods_cat WHERE cat_id='.intval($catid));
        $row = $this->getList('parent_id', array(
            'cat_id' => intval($catid)
        ));
        $parent_id = $row[0]['parent_id'];
        $this->db->exec('DELETE FROM vmc_b2c_goods_cat WHERE cat_id=' . intval($catid));
        $this->db->exec('UPDATE vmc_b2c_goods_cat SET child_count = child_count-1 WHERE cat_id=' . intval($parent_id));
        $this->cat2json();
        return true;
    }
    function get_new_cat($limit) {
        $cat_id = $this->db->select('SELECT cat_id FROM `vmc_b2c_goods`  where cat_id <> \'0\' group by cat_id order by goods_id desc limit 0,' . $limit);
        if (is_array($cat_id)) {
    ...
    function get_cat_name($cat_id) {
        $list = $this->getRow("cat_name", array(
            'cat_id' => $cat_id
        ));

...
class b2c_ctl_admin_sales_order extends desktop_controller
{
        ...
        $this->finder('b2c_mdl_sales_rule_order', array(
            ...
            'actions' => array(
                array(
                    'label' => ('添加规则'),
                    'icon'=>'fa-plus',
                    'href' => 'index.php?app=b2c&ctl=admin_sales_order&act=add',
                ),
            ),
            'base_filter' => array(
                'rule_type' => 'N',
    ...
    private function _editor($rule_id = 0)
    {
        ...
        $this->page('admin/sales/promotion/order.html');
    }
    public function toAdd()
    {
        $this->begin('index.php?app=b2c&ctl=admin_sales_order&act=index');
        $post_data = $_POST;
        $aData = $this->_prepareRuleData($post_data);
    ...
    private function _prepareRuleData($aData)
    {
        ...
        ////////////////////////////// 过滤规则 //////////////////////////////////
        $aResult['conditions'] = empty($aData['conditions']) ? (array(
            'type' => 'combine',
            'conditions' => array(),
        )) : ($aData['conditions']);
        //if(is_null($aResult['conditions'])) $aResult['c_template'] = null;
        $aResult['action_conditions'] = empty($aData['action_conditions']) ? (array(
            'type' => 'product_combine',
            'conditions' => array(),
        )) : ($aData['action_conditions']);
        ////////////////////////////// 优惠方案 //////////////////////////////////

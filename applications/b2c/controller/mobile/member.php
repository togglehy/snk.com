...
class b2c_ctl_mobile_member extends b2c_mfrontpage
{
    public $title = '会员中心';
    ...
    private function get_menu()
    {
        $menu = array(
            ...
            //             'label' => ('商品评价') ,
            //             'ordernum'=>20,
            //             'link'=>array('app'=>'b2c','ctl'=>'mobile_member','act'=>'index')
            //         ) ,
            //         array(
            //             'label' => ('申请售后服务') ,
            //             'ordernum'=>0,
    ...
    protected function output($app_id)
    {
        ...
        $this->page('mobile/member/main.html');
    }
    /**
     * 会员中心首页.
     */
    ...
    public function set_pam_uname($action=false){
        ...
        $user_obj = vmc::singleton('b2c_user_object');
        $redirect_member_index = array('app' => 'b2c','ctl' => 'mobile_member');
        $redirect_here = array('app' => 'b2c','ctl' => 'mobile_member','act' => 'set_pam_uname');
        ...
        $this->page('mobile/member/set_pam_local.html');
    }
    ...
    public function set_pam_mobile($action=false){
        ...
        $user_obj = vmc::singleton('b2c_user_object');
        $redirect_member_index = array('app' => 'b2c','ctl' => 'mobile_member');
        $redirect_here = array('app' => 'b2c','ctl' => 'mobile_member','act' => 'set_pam_mobile');
        $pam_data = $user_obj->get_pam_data('*', $this->member['member_id']);
        if($pam_data['mobile']){
        ...
        $pam_data_schema = app::get('pam')->model('members')->get_schema();
        $this->pagedata['pam_type'] = $pam_data_schema['columns']['login_type']['type'];
        $this->page('mobile/member/set_mobile.html');
    }
    ...
    public function set_pam_email($action=false){
        ...
        $user_obj = vmc::singleton('b2c_user_object');
        $redirect_member_index = array('app' => 'b2c','ctl' => 'mobile_member');
        $redirect_here = array('app' => 'b2c','ctl' => 'mobile_member','act' => 'set_pam_email');
        $pam_data = $user_obj->get_pam_data('*', $this->member['member_id']);
        if($pam_data['email']){
        ...
        if($action == 'save'){
            $email = $_POST['email'];
            $vcode = $_POST['vcode'];
            if(!vmc::singleton('b2c_user_vcode')->verify($vcode, $email, 'signup')){
                $this->splash('error',$redirect_here,'验证码错误！');
            }
        ...
        $pam_data_schema = app::get('pam')->model('members')->get_schema();
        $this->pagedata['pam_type'] = $pam_data_schema['columns']['login_type']['type'];
        $this->page('mobile/member/set_email.html');
    }
    ...
    public function favorite($action = 'list', $gid = false)
    {
        ...
        $redirect_here = array('app' => 'b2c','ctl' => 'mobile_member','act' => 'favorite');
    ...
    public function receiver($action = 'list', $addr_id = false)
    {
        ...
        $member_id = $this->member['member_id'];
        $redirect = array('app' => 'b2c', 'ctl' => 'mobile_member', 'act' => 'receiver');
        switch ($action) {
            case 'set_default':
